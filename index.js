...
async function getLLMJudge(molitSummary, elevatorSummary, baseItem) {
    ...
    const prompt = `
[ROLE]
당신은 규칙을 임의로 변경하거나 추론을 생략할 수 없습니다.
오직 아래 [증거 데이터]를 [판결 기준]에 순서대로 대입하여 하나의 등급만 판정하고,
출력은 반드시 지정된 JSON 형식으로만 반환해야 합니다.
어떤 경우에도 규칙을 위반하면 안 됩니다.
...
[판결 기준 — 절대 변경 불가]
1. [다중이용건축물-피난]:
   - 만약 피난용 승강기 = "있음 (TRUE)" 이면 **무조건** 이 등급.
   - 만약 피난용 승강기 = "없음 (FALSE)" 이면 **절대** 이 등급이 될 수 없음.

2. [다중이용건축물]:
   - 피난용 승강기 = "없음 (FALSE)" 이고,
     (최고층수 ≥ 16) **또는** (가목 연면적 ≥ 5000.00) 이면 해당.
   - 둘 다 거짓이면 해당하지 않음.

3. [일반건축물]:
   - 위 1,2 모두 해당하지 않을 때 해당.

--------------------------------------
[출력 규칙 — 반드시 지킬 것]
1. 출력은 **오직** 다음 JSON 하나만 반환할 것 (추가 텍스트 금지):

{
  "code": "RED" | "BLUE" | "REJECT",
  "decision_text": "다중이용건축물-피난" | "다중이용건축물" | "일반건축물" | "REJECT",
  "reason": "정확한 판결 이유(숫자비교 포함).",
  "checks": {
    "evac_check": "TRUE or FALSE with short reason",
    "floor_check": "TRUE or FALSE with numeric compare",
    "area_check": "TRUE or FALSE with numeric compare"
  }
}

2. **검증 규칙**(모델이 스스로 검사해서 일관성 없으면 REJECT):
   - 만약 evac_check === "FALSE"인데 decision_text === "다중이용건축물-피난" 이면
     반드시 {"code":"REJECT","decision_text":"REJECT",...} 을 반환하라.
   - 각 check(세 항목)는 반드시 TRUE/FALSE로 표기하고 비교 숫자(예: 23764.13 ≥ 5000)를 써야 함.
   - 최종 decision은 판결 기준에 완전 부합해야 함.

3. 절대: 자유 서술, 추가 권고, 법령 인용, 또는 규칙 외 비교를 출력에 포함하지 말 것.
--------------------------------------
