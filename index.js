...
// ============================================================
// 🚨 7. 안전 등급 결정 (Server-Driven Logic) - 오류 수정됨
// ============================================================
function determineSafetyGrade(molitSummary, elevatorSummary, isFallback) {
    ...
    // 승강기 개수 추정 (elevatorSummary는 maxFloor만 있으므로, 실제 count는 상위에서 받아야 함)
    // 하지만 로직 단순화를 위해 maxFloor > 0 이면 승강기가 있다고 간주
    ...
    // [BLUE] 일반 관리 대상 (4시간) - Case 1(정상) & Case 2(Fallback) 모두 포함
    // 승강기 데이터가 있거나, MOLIT 데이터상 2층 이상이면 승강기 보유로 간주
    ...
    // [YELLOW] 확인 필요 (데이터상 승강기 없지만 2층 이상인 경우 - 위에서 BLUE로 흡수했으므로 여기는 1층 이하만 남음)
    // 고객 요청에 따라 2층 이상은 무조건 BLUE로 처리했으므로 YELLOW는 사실상 도달하지 않음.
    // 혹시 모를 예외 처리를 위해 남겨둠.
