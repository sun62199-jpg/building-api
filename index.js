...
**[판단 근거 작성 규칙]**
1.  답변은 한국 건축법상 **가목(특정 용도 연면적 5000㎡ 이상) 또는 나목(16층 이상)** 기준 중 **실제로 해당되는 기준**을 명확히 언급해야 한다.
2.  판단 근거는 단정적인 문장 하나로 구성되어야 하며, 다음과 같은 형식만을 사용해야 한다:
    "이 건물은 다중이용건축물 기준 중 **[해당되는 용도](굵은글씨)**로 해당되고, 연면적이 **[총 연면적 수치]㎡**이기 때문에 다중이용건축물에 해당됩니다."
    또는
    "이 건물은 **[지상층 수치]층** 이상이므로 나목 기준에 해당되어 다중이용건축물에 해당됩니다."
    또는
    "이 건물은 다중이용건축물 기준(가목, 나목)에 해당되지 않습니다."

${JSON.stringify(summary, null, 2)}

출력 예시:
{ "다중이용건축물": "예/아니오", "판단근거": "이 건물은 다중이용건축물 기준 중 **판매시설**로 해당되고, 연면적이 6000㎡이기 때문에 다중이용건축물에 해당됩니다." }
`;
  const response = await openai.chat.completions.create({
    model: "gpt-3.5-turbo",
    messages: [{ role: "user", content: prompt }],
    temperature: 0.1, // 답변 형식 준수를 위해 temperature를 0.1로 약간 올림
  });
