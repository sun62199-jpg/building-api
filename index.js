...
async function generateLLMDescription(gradeInfo, molitSummary, elevatorSummary) {
    ...
    // 🚨 팩트 기반 판단 (Node.js Logic Check)
    const isGaMok = area >= 5000;
    const isNaMok = finalFloor >= 16;

    const prompt = `
    [역할] 당신은 건축법 전문가이자 최종 문구를 작성하는 AI입니다.
    ...
    2. **문구 생성:** 위 판단 결과에 따라 **정확히 해당되는 템플릿 문구 하나**를 'reason' 필드에 삽입하세요. (예: 15층이면 나목 템플릿을 선택하면 안 됨)
    ...
    try {
        ...
        return { decision: resultText, reason: gradeInfo.desc_prefix + " (AI 파싱 오류로 인한 원문 복구 실패)" };
