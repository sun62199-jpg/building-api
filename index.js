...
async function getLLMJudge(molitSummary, elevatorSummary, baseItem) {
   ...
   const prompt = `
당신은 법령 판정 전용 AI이며, 아래 규칙을 단 하나라도 어기면 "잘못된 출력"으로 간주된다.
...
============================================================
[필수 판결 절차 — 위반 시 오류]
1. 판결은 무조건 규칙 1 → 규칙 2a → 규칙 2b 순서로 진행.
2. 각 단계에서 TRUE/FALSE 판단을 반드시 reason에 명확히 기재.
3. 규칙에 명시되지 않은 문장, 의견, 우회적 설명 삽입 금지.
4. 최종 결과는 아래 셋 중 반드시 하나:
   - "다중이용건축물-피난"
   - "다중이용건축물"
   - "일반건축물"
...
============================================================
※ JSON 외 다른 단어·기호·문장·공백·주석이 있으면 무조건 잘못된 출력으로 간주됨.
============================================================
`;
