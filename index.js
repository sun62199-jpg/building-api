...
async function searchAddress(input) {
    ...
    return {
        ...
        rawJuso: juso, // bdMgtSn, detBdNmList 등 공동주택 정보 포함
    };
}

// 5. 일반 건축물 조회 (기존 함수 유지 - 통합 함수에서 사용됨)
async function fetchBuildingRegister(addressInfo) {
    ...
    if (!res.ok) throw new Error(`일반 건축물대장 API 오류: HTTP ${res.status}`);

    let data;
    try {
        data = JSON.parse(text);
    } catch (e) {
        throw new Error("일반 건축물대장 JSON 파싱 실패 → " + text);
    }

    const header = data.response?.header;
    // resultCode "03"은 데이터 없음일 수 있으므로 "00"만 성공으로 처리
    if (!header || header.resultCode !== "00") {
        if (header?.resultCode === "03") {
            return []; // 데이터가 없을 경우 빈 배열 반환
        }
        throw new Error(
            `일반 건축물대장 조회 실패: ${header?.resultMsg || "알 수 없는 오류"}`
        );
    }
...
// 5-1. 🏠 공동주택 표제부 조회 (bdMgtSn 사용)
async function fetchBuildingRegisterApt(bdMgtSn) {
    const url = new URL(
        "https://apis.data.go.kr/1613000/BldRgstHubService/getBrTitleInfoApt"
    );
    const params = {
        serviceKey: MOLIT_KEY,
        bdMgtSn: bdMgtSn,
    ...
    const res = await fetch(url.toString());
    const text = await res.text();
    if (!res.ok) throw new Error(`공동주택 API 오류: HTTP ${res.status}`);

    let data;
    try {
        data = JSON.parse(text);
    } catch (e) {
        throw new Error("공동주택 JSON 파싱 실패 → " + text);
    }

    const header = data.response?.header;
    // resultCode "99" (데이터 없음) 또는 "03"도 처리
    if (!header || header.resultCode !== "00") {
        if (["99", "03"].includes(header?.resultCode)) {
            return [];
        }
        throw new Error(
            `공동주택 조회 실패: ${header?.resultMsg || "알 수 없는 오류"}`
        );
    }
...
function deduplicateItems(items) {
    ...
    items.forEach(item => {
        // mgmBldrgstPk가 가장 확실한 고유 식별자입니다.
        const key = item.mgmBldrgstPk;
        if (key) {
...
// 5-2. 🔄 통합 데이터 조회 (fetchBuildingRegister 대체)
async function fetchBuildingData(addressInfo) {
    const rawJuso = addressInfo.rawJuso;
    const bdMgtSn = rawJuso.bdMgtSn;

    let itemsApt = [];
    let itemsGeneral = [];
    let allItems = [];

    // 1. 🚨 공동주택 우선 조회 시도 (bdMgtSn이 있을 경우)
    if (bdMgtSn) {
        console.log(`[DEBUG] 공동주택 관리번호(${bdMgtSn})로 조회 시도.`);
        try {
            itemsApt = await fetchBuildingRegisterApt(bdMgtSn);
        } catch (e) {
            console.warn(`[WARN] 공동주택 조회 실패 (${e.message}).`);
        }
    }

    // 2. 🚨 일반 건축물 조회 시도 (단지 내 부속 건물 포함)
    console.log(`[DEBUG] 일반 건축물 지번(${addressInfo.bun}-${addressInfo.ji})으로 조회 시도.`);
    try {
        itemsGeneral = await fetchBuildingRegister(addressInfo);
    } catch (e) {
        console.warn(`[WARN] 일반 건축물 조회 실패 (${e.message}).`);
    }

    // 3. 결과 병합 및 중복 제거
    allItems = [...itemsApt, ...itemsGeneral];
    
    if (allItems.length === 0) {
        return [];
    }
    
    // mgmBldrgstPk 기준으로 중복 제거
...
function buildSummary(items) {
    // 공동주택을 필터링 항목에서 제외하고, 16층 이상 여부 판단에만 사용하도록 로직 변경 필요
    ...
    // 전체 건물 목록에서 최고층 계산 (공동주택 포함)
    const 최고지상층수 = items.length
        ? Math.max(...items.map(it => Number(it.grndFlrCnt) || 0))
        : 0;
...
async function kakaoSummaryHandler(req, res) {
    try {
        ...
        // 🚨 통합 함수 호출
        const items = await fetchBuildingData(addressInfo);
        ...
        if (items.length === 0) {
             return res.json({
                ...
                template: {
                    outputs: [{
                        simpleText: {
                            text: `⚠️ 죄송합니다. "${cleanAddr}"에 대한 건축물대장 정보가 없습니다.\n\n(주소 검색은 성공했으나 건축물대장이 조회되지 않음)`,
                        }
                    }]
                }
            });
