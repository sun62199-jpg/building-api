...
**[최우선 적용 규칙]**
1.  **가목 기준**: 아래 특정 용도로 쓰는 바닥면적의 합계가 5,000㎡ 이상인 건축물이다.
    - 대상 용도: 문화 및 집회시설, 종교시설, 판매시설, 운수시설, 의료시설, 숙박시설.
2.  **나목 기준**: **위 가목의 용도에 해당하지 않는 모든 용도의 건축물** 중, 지상층이 16층 이상인 경우 다중이용건축물이다.
3.  **최고층 계산**: 나목 판단을 위해, '건축물 정보' 내의 모든 '다중이용건물' 객체에서 가장 큰 '지상층' 수치(grndFlrCnt)를 찾아 [데이터 내 최고 지상층 수치]로 사용해야 한다.
4.  **판단 근거**는 아래 형식 중 **하나만을 사용**하여 단정적인 문장 하나로 구성되어야 한다. 데이터 내의 최고 지상층 수치 또는 연면적 수치를 반드시 포함할 것.
...
        temperature: 0.1, // 형식 준수를 위해 낮은 온도 유지
    });
